...
Configurazioni di replica vengono utilizzate per configurare il servizio di replica è responsabile dell'esecuzione lo stato di Provider dello stato dell'attore affidabilità, la replica e mantenere lo stato in locale.
...
|CheckpointThresholdInMB|MB|200|Quantità di spazio di file di log trascorso il quale viene eseguito un checkpoint lo stato.|
...
|OptimizeLogForLowerDiskUsage|Valore booleano|vero|Quando è true, il log è configurato in modo che il file di log dedicato della replica viene creato tramite un file caricato in memoria bassa NTFS. Consente di ridurre l'utilizzo di spazio su disco effettivo per il file. Quando è false, il file viene creato con allocazioni fisse, che forniscono che i migliori prestazioni di scrittura.|
