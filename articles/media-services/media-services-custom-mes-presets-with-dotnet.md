<properties 
    pageTitle="Avanzate codifica con Media Encoder Standard | Microsoft Azure" 
...
>[AZURE.NOTE]L'impostazione di **SceneChangeDetection** di seguito possono preimpostato essere impostata su true se si esegue la codifica a un singolo velocità video. Se si codifica a un video con più velocità e impostare **SceneChangeDetection** su true, il codificatore restituirà un errore.  
...
>
>I file video di origine e il file di sovrapposizione deve essere bene stesso.
...
Per impostazione predefinita, se si invia un input al codificatore che contiene solo video e audio non quindi bene output conterrà file contenenti dati solo video. Alcuni lettori potrebbero non riuscire a gestire tali flussi di output. Per forzare il codificatore per aggiungere un brano invisibile per l'output in questo scenario, è possibile utilizzare questa impostazione.
