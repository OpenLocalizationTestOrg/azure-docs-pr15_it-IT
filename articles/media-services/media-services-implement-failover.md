<properties 
    pageTitle="Implementazione failover streaming scenario | Microsoft Azure" 
    description="In questo argomento viene illustrato come implementare failover streaming scenario." 
...
#<a name="implementing-failover-streaming-scenario"></a>Implementazione failover streaming scenario

Questa procedura viene illustrato come copiare il contenuto (BLOB) da una risorsa in un altro per gestire la ridondanza per streaming su richiesta. In questo scenario è utile per i clienti che da configurare la rete CDN a failover tra due Data Center nel caso di un'interruzione in uno del data center.
In questa procedura dettagliata utilizza SDK dei servizi multimediali di Microsoft Azure, all'API REST di servizi multimediali di Microsoft Azure e Azure archiviazione SDK per illustrare le attività seguenti.

1. Configurare un account di servizi multimediali in "Data Center A".
1. Caricare un file mezzanine in una risorsa di origine.
1. Codificare la risorsa nei file di velocità bit multiplo MP4. 
1. Creare un locator sa di sola lettura per la risorsa di origine avere accesso in lettura il contenitore nell'account di archiviazione associata all'attività di origine.
1. È possibile ottenere il nome del contenitore di bene origine dal locator di SA sola lettura creato nel passaggio precedente. Queste informazioni sono necessarie per la copia BLOB tra gli account di archiviazione (descritte nell'argomento).
1. Creare un indicatore di posizione di origine per la risorsa che ha creata l'attività di codifica. 

Quindi, per la gestione del failover:

1. Configurare un account di servizi multimediali in "Dati Center B".
1. Creare una risorsa di destinazione vuoto nell'account di servizi multimediali di destinazione.
1. Creare un indicatore di posizione SA scrittura per risorsa vuota di destinazione accesso in scrittura per il contenitore nell'account di archiviazione associato risorsa di destinazione di destinazione.
1. Utilizzare lo spazio di archiviazione Azure SDK per copiare BLOB (file di risorse) tra l'account di archiviazione di origine in "Data Center A" e account di archiviazione di destinazione in "Dati Center B" (questi account di archiviazione sono associati alle risorse di interesse).
1. Associare BLOB (file di risorse) che sono stati copiati nel contenitore di destinazione blob con la risorsa di destinazione. 
1. Creare un indicatore di posizione di origine per la risorsa della "Dati Center B" e specificare locator Id generati per la risorsa della "Data Center A". 
1. Si ottiene così URL di trasmissione in cui i percorsi relativi degli URL sono gli stessi (solo la base URL sono diversi). 
 
Quindi, per gestire le interruzioni, è possibile creare una rete CDN nella parte superiore di questi indicatori di origine. 

Le considerazioni seguenti:

- La versione corrente del Media Services SDK non supporta la creazione di un indicatore di posizione con un id locator specificato. Per ottenere questa operazione, verrà usata all'API REST di servizi di supporto.
- La versione corrente del Media Services SDK non supporta la generazione a livello di programmazione informazioni IAssetFile che associazione di un bene con i file di risorse. Per ottenere questa operazione, verrà usata l'API REST di CreateFileInfos Media Services. 
- Risorse di archiviazione crittografato (AssetCreationOptions.StorageEncrypted) non sono supportati per la replica (poiché la chiave di crittografia sarà diversa in entrambi gli account servizi multimediali). 
- Se si vuole sfruttare imballaggio dinamico, è prima necessario ottenere almeno un flusso su richiesta unità riservata. Per ulteriori informazioni, vedere [Dinamicamente imballaggio risorse](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]È possibile utilizzare i servizi multimediali [Dello strumento di replica](http://replicator.codeplex.com/) come un'alternativa all'implementazione di uno scenario di trasmissione failover manualmente. Questo strumento consente di replicare risorse tra due account servizi multimediali.

##<a name="prerequisites"></a>Prerequisiti
 
- Due account servizi multimediali di una sottoscrizione di Azure nuovo o esistente. Informazioni su [come creare un Account di servizi di supporto](media-services-portal-create-account.md).
- Sistemi operativi: Windows 7, Windows 2008 R2 o Windows 8.
- .NET framework 4.5 o .NET Framework 4.
- Visual Studio 2010 SP1 o versione successiva (Professional, Premium, Ultimate o Express).
 
##<a name="set-up-your-project"></a>Configurare il progetto

In questa sezione verranno creare e configurare un progetto di applicazione Console c#.

1. Utilizzare Visual Studio per creare una nuova soluzione che contiene il progetto di applicazione Console c#. Immettere HandleRedundancyForOnDemandStreaming per il nome e quindi fare clic su OK.
1. Creare la cartella SupportFiles allo stesso livello del file di progetto HandleRedundancyForOnDemandStreaming.csproj. Nella cartella SupportFiles creare cartelle OutputFiles e MP4Files. Copiare un file MP4 nella cartella MP4Files (in questo esempio che viene utilizzato il file BigBuckBunny.mp4). 
...
1. Aggiungere gli altri riferimenti necessari per il progetto: Configuration System.Runtime.Serialization e System.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Aggiungere codice che gestisce la ridondanza per streaming su richiesta.



1. Aggiungere i campi seguenti a livello di classe alla classe Program.
...
1. Sostituire la definizione del metodo principali predefinita con quella riportata di seguito:
...
1. Definizioni di metodo chiamati dai principali sono definite sotto.
...
È ora possibile usare un responsabile di traffico per indirizzare le richieste tra due centri dati e quindi failover nel caso di qualsiasi interruzioni.
