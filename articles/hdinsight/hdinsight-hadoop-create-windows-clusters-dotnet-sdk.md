<properties
   pageTitle="Creare cluster basato su Windows Hadoop in HDInsight mediante .NET SDK | Microsoft Azure"
    description="Informazioni su come creare cluster HDInsight per Azure HDInsight con .NET SDK."
   services="hdinsight"
   documentationCenter=""
   tags="azure-portal"
   authors="mumian"
...
# <a name="create-windows-based-hadoop-clusters-in-hdinsight-using-net-sdk"></a>Creare cluster basato su Windows Hadoop in HDInsight mediante .NET SDK

[AZURE.INCLUDE [selector](../../includes/hdinsight-selector-create-clusters.md)]


Informazioni su come creare cluster HDInsight mediante .NET SDK. Per la creazione di altri cluster caratteristiche e strumenti fare clic su Seleziona scheda nella parte superiore di questa pagina oppure [metodi di creazione Cluster](hdinsight-provision-clusters.md#cluster-creation-methods).
...
HDInsight .NET SDK fornisce raccolte client .NET che rendono più semplice per l'uso con HDInsight da un'applicazione di .NET Framework. Seguire le istruzioni seguenti per creare un'applicazione di console di Visual Studio e incollare il codice per la creazione di un cluster.

L'applicazione richiede un gruppo di risorse Azure e l'account di archiviazione predefinito.  [Appendice](#appx-a-create-dependent-components) fornisce un script di PowerShell per creare i componenti dipendenti.

**Per creare un'applicazione di console di Visual Studio**

1. Creare una nuova applicazione console c# in Visual Studio.
2. Eseguire il seguente comando Nuget nella console di Nuget pacchetto.
...
7. Premere **F5** per eseguire l'applicazione. Una finestra della console deve aprire e visualizzare lo stato dell'applicazione. È inoltre venga chiesto di immettere le credenziali dell'account Azure. È possibile richiedere alcuni minuti per creare un cluster di HDInsight.
...
##<a name="appx-a-create-dependent-components"></a>Componenti dipendenti crea AppX A

Il PowerShell Azure seguenti può essere script consente di creare componenti dipendenti necessari per l'applicazione .NET in questa esercitazione.
