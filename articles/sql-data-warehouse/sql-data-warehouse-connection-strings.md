...
# <a name="drivers-for-azure-sql-data-warehouse"></a>Driver di SQL Azure Data Warehouse

È possibile connettersi a SQL Data Warehouse con numerosi protocolli di applicazione diverso, ad esempio, [ADO.NET][], [ODBC][], [PHP][] e [JDBC][]. Di seguito sono riportati alcuni esempi di stringhe di connessione per ogni protocollo.  Utilizzare il portale di Azure per creare la stringa di connessione.  Per creare la stringa di connessione tramite il portale di Azure, passare al blade database, in *Essentials* fare clic su *Mostra le stringhe di connessione di database*.
