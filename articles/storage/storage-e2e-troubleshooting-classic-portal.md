<properties 
    ...
    description="Un'esercitazione-to-end risoluzione dei problemi di Azure archiviazione Analitica, AzCopy e Analyzer messaggio Microsoft" 
    services="storage" 
...
In questa esercitazione fornisce un'esplorazione pratica di uno scenario di risoluzione dei problemi-to-end. Per una Guida dettagliata concettuale di risoluzione dei problemi di applicazioni Azure dello spazio di archiviazione, vedere [Monitor, diagnosticare e risolvere i problemi di spazio di archiviazione di Microsoft Azure](storage-monitoring-diagnosing-troubleshooting.md). 
...
- **Spazio di archiviazione azure Analitica**. [Azure archiviazione Analitica](http://msdn.microsoft.com/library/azure/hh343270.aspx) offre metriche e la registrazione per lo spazio di archiviazione di Azure.
...
- **Azure portale classica**. È possibile configurare la registrazione e metriche per il proprio account di archiviazione nel [Portale classica Azure](https://manage.windowsazure.com). È anche possibile visualizzare i grafici che mostrano le prestazioni dell'applicazione nel tempo e configurare avvisi per comunicare se l'applicazione esegue in modo diverso dal previsto per una metrica specificata. 
    
    Per informazioni sulla configurazione di monitoraggio nel portale classica di Azure, vedere [Monitor di un account di archiviazione nel portale di Azure](storage-monitor-storage-account.md) .
...
- **Messaggio Microsoft Analyzer**. Messaggio Analyzer è uno strumento fruisce dei file di log e Visualizza dati log in un formato visivo che facilita la filtro, ricerca e raggruppare i dati di log in set utile che consente di analizzare gli errori e problemi di prestazioni. Vedere [Guida di operativo Microsoft messaggio Analyzer](http://technet.microsoft.com/library/jj649776.aspx) per ulteriori informazioni sull'analisi di messaggio.

## <a name="about-the-sample-scenario"></a>Sullo scenario di esempio

Per questa esercitazione verrà esaminato uno scenario in cui metriche di archiviazione di Azure indica un tasso di successo percentuale bassa per un'applicazione che chiama lo spazio di archiviazione Azure. Le unità di misura metriche tasso successo percentuale bassa (come illustrato come **PercentSuccess** nel portale classica di Azure e nelle tabelle metriche) tiene traccia delle operazioni eseguite con esito positivo, ma che restituiscono un codice di stato HTTP è superiore a 299. Nei file di log di archiviazione sul lato server queste operazioni sono registrate con lo stato transazione **ClientOtherErrors**. Per ulteriori informazioni sulle unità di misura metriche successo percentuale bassa, vedere [metriche mostrano PercentSuccess bassa o voci di log analitica avere operazioni con lo stato di ClientOtherErrors transazione](storage-monitoring-diagnosing-troubleshooting.md#metrics-show-low-percent-success).
...
In questo scenario, si vedrà un tasso di successo percentuale bassa trattarsi inferiore al 100%. È possibile scegliere un livello di metriche diverso, tuttavia, in base alle proprie esigenze. È consigliabile durante la verifica dell'applicazione, creare una tolleranza prevista per le metriche di prestazioni chiave. Ad esempio, è possibile stabilire, in base ai test, l'applicazione deve disporre di un tasso di successo percentuale coerente di 90% o 85%. Se i dati metriche indicano che l'applicazione è la deviazione da tale numero, quindi è possibile esaminare che possono causare l'aumento. 

Per questo scenario di esempio, una volta è stata stabilita che unità di misura metriche tasso percentuale di completamento è inferiore al 100%, esamineremo i log per trovare gli errori che sono correlate a metriche e usarli per stabilire la causa la percentuale di successo percentuale inferiore. Vengono illustrate in modo specifico alla errori nell'intervallo 400. È quindi in modo approfondito più da vicino 404 errori (non disponibile).

### <a name="some-causes-of-400-range-errors"></a>Alcune cause degli errori di intervallo 400

Gli esempi riportati di seguito illustra un esempio di alcuni degli errori 400 intervallo le richieste di archiviazione Blob Azure e le possibili cause. Uno di questi errori, come gli errori in intervallo 300 e l'intervallo 500 può contribuire a un tasso di successo percentuale bassa. 

Si noti che non sono elencati di seguito completata. Visualizzare [lo stato e codici di errore](http://msdn.microsoft.com/library/azure/dd179382.aspx) per informazioni dettagliate sugli errori di archiviazione Azure generali e informazioni sugli errori specifici per ognuno dei servizi di archiviazione.
...
- Si verifica se si utilizza una chiamata API che consente di creare il contenitore o blob dopo aver verificato se esiste. APIs CreateIfNotExists effettuare una chiamata di testa per verificare l'esistenza del contenitore o blob; Se non esiste, viene restituito un errore 404 e quindi secondo inserire è una chiamata per scrivere il contenitore o blob.

**Codice di stato 409 esempi (conflitto)**

- Si verifica se si usa un'API crea per creare un nuovo contenitore o blob, senza prima verificare la presenza ed esiste già un contenitore o blob con lo stesso nome. 
- Se si desidera eliminare un contenitore e si tenta di creare un contenitore di nuovo con lo stesso nome prima del completamento dell'operazione di eliminazione.
...
- **Registro server**, che viene creata quando si attiva la registrazione di archiviazione Azure. Il registro server contiene dati relativi a ogni operazione denominata rispetto a uno dei servizi di archiviazione Azure - blob, code, tabella e file. Registro server viene indicato il tipo di operazione è stato chiamato e il codice di stato è stati restituiti, nonché altri dettagli sulla richiesta e risposta.
- Il **Registro di client .NET**, che viene creato quando si attiva la registrazione lato client all'interno dell'applicazione .NET. Il registro client include informazioni dettagliate su come il client prepara la richiesta e riceve ed elabora la risposta. 
- Il **registro traccia di rete HTTP**, che consente di raccogliere dati sui dati di richiesta e risposta HTTP/HTTPS, inclusi per operazioni in archiviazione Azure. In questa esercitazione, è necessario generare la traccia di rete tramite Analyzer messaggio.

### <a name="configure-server-side-logging-and-metrics"></a>Configurare la registrazione lato server e metriche

Prima di tutto, è necessario configurare la registrazione di archiviazione Azure e metriche, in modo che è stata creata dall'applicazione client per analizzare i dati. È possibile configurare la registrazione e la metrica in vari modi, tramite il [Portale classica Azure](https://manage.windowsazure.com)tramite PowerShell, o a livello di programmazione. Per informazioni dettagliate sulla configurazione di registrazione e metriche, vedere [attivazione metriche di archiviazione e la visualizzazione dati metriche](http://msdn.microsoft.com/library/azure/dn782843.aspx) e [Abilitazione della registrazione di spazio di archiviazione e accesso ai dati di Log](http://msdn.microsoft.com/library/azure/dn782840.aspx) .

**Tramite il portale classico Azure**

Per configurare la registrazione e la metrica per l'account di archiviazione tramite il portale, seguire le istruzioni disponibili in [Monitor di un account di archiviazione nel portale di Azure](storage-monitor-storage-account.md).

> [AZURE.NOTE] Non è possibile impostare minute metriche tramite il portale classica Azure. Tuttavia, è consigliabile impostare li ai fini di questa esercitazione e per l'analisi dei problemi di prestazioni con l'applicazione. È possibile impostare minute metriche tramite PowerShell come indicato di seguito, oppure a livello di programmazione o tramite il portale classica Azure.
>
> Si noti che il portale classica Azure non è possibile visualizzare metriche minute, solo orarie metriche. 
...
3. Impostare l'account di archiviazione predefinito per account di archiviazione in uso per l'esercitazione eseguendo questi comandi nella finestra di PowerShell:
...
![Configurare Fiddler opzioni](./media/storage-e2e-troubleshooting-classic-portal/fiddler-options-1.png)

Per l'esercitazione raccogliere e prima di salvare una traccia di rete nell'analizzatore messaggio e quindi crea una sessione di analisi per analizzare la traccia e i log. Per raccogliere una traccia di rete nell'analizzatore messaggio:

1. Nel messaggio di analisi, selezionare **File | Analisi rapida | Non crittografato HTTPS**.
...
4. Selezionare il collegamento **Configure** a destra del provider ETW **WebProxy di Pef Microsoft** .
...
>[AZURE.NOTE] Dopo aver completato la raccolta traccia di rete, si consiglia di ripristinare le impostazioni modificate in Fiddler per il traffico HTTPS decrittografia. Nella finestra di dialogo Opzioni Fiddler, deselezionare le caselle di controllo **Acquisire connette HTTPS** e **Decrittografare il traffico HTTPS** .
...
## <a name="review-metrics-data-in-the-azure-classic-portal"></a>Esaminare i dati metriche nel portale classica di Azure

Dopo l'applicazione è stata eseguita per un periodo di tempo, è possibile esaminare i grafici metriche visualizzati nel portale classica di Azure per osservare come l'esecuzione del servizio. Prima di tutto è aggiungerà unità di misura metriche **Percentuale di successo** alla pagina monitoraggio:

1. Passare a Dashboard per l'account di archiviazione nel [Portale classica di Azure](https://manage.windowsazure.com), quindi selezionare **Monitor** per visualizzare la pagina monitoraggio.
2. Fare clic su **Aggiungi metriche** per visualizzare la finestra di dialogo **Scegliere metriche** .
3. Scorrimento fino a trovare il gruppo di **Percentuale di successo** , espanderlo, quindi selezionare **aggregazione**, come illustrato nella figura seguente. Questa metrica aggrega i dati di percentuale di successo da tutte le operazioni di Blob.

![Scegliere i criteri di misurazione](./media/storage-e2e-troubleshooting-classic-portal/choose-metrics-portal-1.png)

Nel portale classica di Azure, si vedrà ora **Percentuale di successo** del grafico monitoraggio, insieme a eventuali altre metriche siano state aggiunte (fino a sei può essere visualizzato nel grafico in una sola volta). Nella figura riportata di seguito, è possibile vedere che la percentuale di successo percentuale è leggermente inferiore al 100%, ovvero lo scenario in modo approfondito Avanti per l'analisi dei registri nell'analizzatore messaggio:

![Grafico metriche nel portale](./media/storage-e2e-troubleshooting-classic-portal/portal-metrics-chart-1.png)
...
> [AZURE.NOTE] Potrebbe richiedere del tempo per le metriche di visualizzare i dati nel portale classica di Azure dopo l'abilitazione metriche di archiviazione. In questo modo orarie metriche per dell'ora precedente non vengono visualizzate nel portale classica di Azure fino a quando non è trascorso, l'ora corrente. Inoltre, minute metriche non vengono visualizzate nel portale classica di Azure. Pertanto a seconda del quando si attiva la metrica, potrebbe richiedere fino a due ore per visualizzare dati di metrica.
...
Per ulteriori informazioni sui registri lato server download, vedere [download registrazione lo spazio di archiviazione dati di log](http://msdn.microsoft.com/library/azure/dn782840.aspx#DownloadingStorageLogginglogdata). 

## <a name="use-microsoft-message-analyzer-to-analyze-log-data"></a>Utilizzare Microsoft messaggio Analyzer per analizzare i dati di log
...
4. Fare clic su **Sincronizza tutti visualizzati gli elementi** per installare i beni di spazio di archiviazione Azure. Le risorse disponibili includono:
    - **Archiviazione azure colore regole:** Regole di colore dello spazio di archiviazione Azure consentono di definire filtri speciali che usano colori, il testo e gli stili dei caratteri per evidenziare i messaggi che contengono informazioni specifiche in una traccia.
    - **Archiviazione azure grafici:** Azure lo spazio di archiviazione sono grafici predefiniti che server log dati del grafico. Si noti che per utilizzare grafici di archiviazione Azure al momento, è possono caricare solo il registro del server nella griglia di analisi.
...
![Messaggio Analyzer Asset Manager](./media/storage-e2e-troubleshooting-classic-portal/mma-start-page-1.png)
...
![Configurare la sessione di messaggistica Analyzer](./media/storage-e2e-troubleshooting-classic-portal/configure-mma-session-1.png)
...
![Dal menu Layout di visualizzazione](./media/storage-e2e-troubleshooting-classic-portal/view-layout-menu.png)
...
![Layout di visualizzazione di archiviazione Azure](./media/storage-e2e-troubleshooting-classic-portal/view-layout-client-request-id-module.png)
...
![Layout di visualizzazione di archiviazione Azure](./media/storage-e2e-troubleshooting-classic-portal/color-rules-menu.png)
...
![Layout di visualizzazione di archiviazione Azure](./media/storage-e2e-troubleshooting-classic-portal/400-range-errors1.png)
...
![Layout di visualizzazione di archiviazione Azure](./media/storage-e2e-troubleshooting-classic-portal/404-filtered-errors1.png)
...
![Server filtrato e registri di traccia di rete](./media/storage-e2e-troubleshooting-classic-portal/server-filtered-404-error.png)
...
![Con registro client 404 errori](./media/storage-e2e-troubleshooting-classic-portal/client-log-analysis-grid1.png)
