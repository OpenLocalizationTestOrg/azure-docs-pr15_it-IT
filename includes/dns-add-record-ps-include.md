...
In questo esempio è usare il nome del set di record "@" per creare un record MX in apice zona (in questo caso, "contoso.com"). Si tratta comune per i record MX.
...
Se si sta creando un record SRV nella radice di un'area, è sufficiente specificare *Service* e *_protocol* nel nome del record. Non è necessario includere "@" nel nome del record.
...
### <a name="create-a-txt-record-set-with-a-single-record"></a>Creare un record TXT set con un singolo record
